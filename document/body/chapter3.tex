...
environments to reach a better performance when executing \ac{ML} inference 
tasks. In particular,
...
Rather than focusing on partitioning a single monolithical model, these
...
\begin{description}[style=unboxed, leftmargin=1cm, font=\bfseries]
    ...
    \item[N/A] No article was yet reviewed in this category.
...
The approaches reviewed in this category are based on processing an initial 
portion of the inference task on devices closer to the end-user and the 
remaining portion on one or more devices on a higher layer, 
further away from the end-user device.

\begin{description}[style=unboxed, leftmargin=1cm, font=\bfseries]

    \item [CoFANN \cite{tran-van_cofann_2025}]
    works out the trade-off between model accuracy and efficiency by employing
    ...
    The network devices are benchmarked in an initial phase, allowing to
    estimate the time needed for each device on each step of the model.
    The framework goes on to employ a differentiable search algorithm to find
    optimal inference strategies for allocating \ac{DNN} tasks to the 
    computing devices.   
    ...
    matching theory for joint partitioning and task offloading. This project 
    ...
    fully connected and convolutional layers. This framework ponders to partition 
    the inference task and then divides it according to the network's condition.
    It should be stated that the network is expected to be homogenous.
...
system to devices locally, avoiding requiring computing power from nodes closer
to the cloud.

\begin{description}[style=unboxed, leftmargin=1cm, font=\bfseries]

    \item[CAS \cite{wang_context-aware_2021}]
    is a framework that succeeds in adapting a given model in real-time, upon a
    network context change. It generates a "partition state graph", containing 
    the set of possible partition, given a
    ...
    of the network, taking note on possible constrained devices. This framework
    creates a fast adapting system, responding to network flutters, keeping the
    active deployment optimized.
    ...
    heterogeneous edge resources and \ac{CNN} models, making its objective to
    maximize throughput.
    ...
    The authors also allow the network to be based on devices with homogenous 
    computation times or heterogenous, given the computation time estimations
...
To better uniformalize the following syntesis, we will recognize previously 
reffered user, end devices and similarly constrained devices as our defitinion 
of far-edge devices. Likewise, the intermediate devices, placed above on the
...
frameworks, for the next subsection, we will be analysing how the presented 
frameworks strategize the scheduling and communications between their devices.
In this section, the frameworks will be separated into two classes, heuristical 
and reinforcement learning scheduling approaches.

\subsection{Heuristics} \label{sec:se321}

Predicting the assignments that lead to the most efficient processing of any
given inference task has been revealed to be a highly complex problem. In this
way, several works have relied on heuristical approaches to simplify the problem
without risking to lose a big part of the desired efficiency. In this subsection
we will be reviewing the framework that rely on heuristics, which heuristics are
...
\begin{description}[style=unboxed, leftmargin=1cm, font=\bfseries]
    ...
    are gathered on a previous stage and taken into account. Network conditions
    are monitored in real-time, allowing the framework to strive for an optimal
    partitioning and deployment, adapting to network changes. 
    ...
    time and data transfers by search throughly a continuous search adapted from
    factors in the inference process. To minimize the data transfer, the initial
    raw data is processed in the end device and condensed as a intermediate 
    feature tensor to be wirelessly transmited to a edge server. The result is
    then sent back to the end device to be logged. 
    ...
    the benefit obtained by executing a given task on that device. This matching is
    further improved by proposing swapping one of the fog nodes between any two
    user nodes.

    \item[\textcite{p_dai_joint_2025}]
    relies on device placement to assign the model partitions to the devices.
    The feature extractor segment of the \ac{CNN} models are distributed along the
    network, while the classifier, as it requires less computation, is assigned to
    a singular device. Intermediate inference results are exchanged between devices
    through a wireless link, using a "OFDMA-based wireless transmission model".
    This solution assumes that the pipeline supports asynchronous communication.
    %rever OFDMA (citação/sigla)

    \item[\textcite{w_miao_adaptive_2020}]
    minimizes the inference time relying on estimations of the computing time
    needed at each step. Since the inference is reckoned to take as long as the
...
\subsection{Reinforcement Learning} \label{sec:se322}
...
\hline
\textbf{Strategy} & \textbf{Scheduling Approach} & \textbf{Optimized Metrics} & \textbf{Communication Method} & \textbf{Tolerates Node Heterogeneity} \\
\hline
CAS \cite{wang_context-aware_2021} & Heuristical & Inference Time & Wireless link (3G, 4G) & \ding{51} \\
\hline
CoFANN \cite{tran-van_cofann_2025} & Heuristical & Inference Time, Data Transfered & Wireless link & \ding{51} \\
\hline
DINA \cite{t_mohammed_distributed_2020} & Heuristical & Inference Time, Resource Utilization & Wireless link & \ding{55} \\
\hline
\textcite{p_dai_joint_2025} & Heuristical & Device Placement & OFMDA-based links & \ding{51} \\
\hline
...
&  
    Proposal for a network context-aware partitioning system, adapting to network
    changes in real-time.
\\
\hline
CoFANN \cite{tran-van_cofann_2025} 
& 
    Proposes a bi-partitioned system that uses detectors in the network to compute 
    initial inference. The framework relies on only two devices, one on-site and
    one edge server. 
&  
    Employment of a gradient descent based search algorithm to detect the optimal
    inference strategy, to minimize latency and data transfer.  
\\
\hline
DINA \cite{t_mohammed_distributed_2020} 
& 
    Employs a bi-hierarchycal fog computing architecture to execute inference tasks.
    Deploys partitioned \ac{DNN} models with \ac{DAG} topologies to homogenous
    devices. Adapts to reduce execution time in varying network conditions.
&  
    Extension of a local inference task to a distributed edge device network.
    Partition and compression techniques for \ac{CNN} models.
    Matching theory for scheduling purposes.
\\
\hline
\textcite{p_dai_joint_2025} 
& 
    Widely applicable framework for collaborative inference in edge networks,
    optimizing throughput and adapting partitioning according each device's 
    capabilities. The framework has been tested to be a highly scalable,
    robust and efficient solution.
& 
    Partition and parallelization techniques for \ac{CNN} models.
    Device placement and network topology considerations for scheduling. 
\\
\hline
...
& 
    ...
    adapts to heterogenous devices, balancing the load between them to
    minimize the overall inference time.
& 
    Load balancing approach for heterogenous devices.
\\
\hline
\end{tabularx}
\vspace{2mm}
